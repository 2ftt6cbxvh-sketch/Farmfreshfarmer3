...
const POLICIES: PolicyDef[] = [
  {
    ...
    contentHtml: `
      ...
      <p>2. This document is published in accordance with the provisions of Rule 3 (1) of the Information Technology (Intermediaries guidelines) Rules, 2011 that require publishing the rules and regulations, privacy policy and Terms of Use for access or usage of domain name https://www.farmfreshfarmer.com/privacy ('Website'), including the related mobile site and mobile application (hereinafter referred to as 'Platform').</p>
      <p>3. The Platform is owned by FarmFreshFarmer, a company incorporated under the Companies Act, 1956 with its registered office at Vijayawada , Andhra Pradesh (hereinafter referred to as ‘Platform Owner’, 'we', 'us', 'our')..</p>
      ...
      <p class="font-bold">6. ACCESSING, BROWSING OR OTHERWISE USING THE PLATFORM INDICATES YOUR AGREEMENT TO ALL THE TERMS AND CONDITIONS UNDER THESE TERMS OF USE, SO PLEASE READ THE TERMS OF USE CAREFULLY BEFORE PROCEEDING..</p>
      ...
      <div class="pl-4 space-y-3">
        ...
        <p>13. All disputes arising out of or in connection with these Terms shall be subject to the exclusive jurisdiction of the courts in and .</p>
  ...
  },
  {
    ...
    contentHtml: `
      ...
      <div class="border-t border-card-border pt-4 mt-6 space-y-2 text-sm">
        <h3 class="font-bold text-foreground text-base">Grievance Officer</h3>
        ...
        <p class="pt-2"><strong>Contact us:</strong></p>
        <p><strong>Phone:</strong> +91 79 89 79 3669 &nbsp;|&nbsp; <strong>Time:</strong> Monday - Friday(9:00 - 22:00)</p>
      </div>
    `
  },
  {
    ...
    contentHtml: `
      ...
      <div class="pl-4 space-y-3">
        ...
        <p>5. In case of any refunds approved by FarmFreshFarmer, it will take 7 days for the refund to be processed to you.</p>
      </div>
    `
  },
  {
  ...
  },
  {
    ...
    contentHtml: `
      <div class="border border-card-border rounded-xl p-5 space-y-2">
        <h3 class="font-bold text-foreground text-base">Grievance Officer</h3>
        <p><strong>Name of the Officer:</strong> PTV Ramachandra Raju</p>
        <p><strong>Designation:</strong> Grievance Officer</p>
        <p><strong>Name and Address of the Company:</strong> FarmFreshFarmer, Vijayawada, Andhra Pradesh</p>
...
function buildPolicyPageHtml(indexTemplate: string, policy: PolicyDef): string {
  const navHtml = `
    <div class="flex items-center gap-1.5 overflow-x-auto pb-3 mb-6 border-b border-card-border/80 no-scrollbar">
      <a href="/terms" class="px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${policy.slug === 'terms' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-secondary/60 text-muted-foreground'}">Terms & Conditions</a>
      <a href="/privacy" class="px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${policy.slug === 'privacy' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-secondary/60 text-muted-foreground'}">Privacy Policy</a>
      <a href="/refund-policy" class="px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${policy.slug === 'refund-policy' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-secondary/60 text-muted-foreground'}">Refund and Cancellation policy</a>
      <a href="/return-policy" class="px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${policy.slug === 'return-policy' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-secondary/60 text-muted-foreground'}">Return Policy</a>
      <a href="/shipping-policy" class="px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${policy.slug === 'shipping-policy' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-secondary/60 text-muted-foreground'}">Shipping Policy</a>
    </div>
  `;

  const bodyHtml = `
    <div class="min-h-screen flex flex-col bg-background text-foreground overflow-x-hidden">
      <header class="border-b border-sidebar-border bg-sidebar text-sidebar-foreground px-4 py-3 sticky top-0 z-40">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
          <a href="/" class="font-serif text-xl font-bold text-emerald-500">FarmFreshFarmer</a>
          <a href="/" class="text-xs font-bold text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded-full">← Back to Store</a>
        </div>
      </header>
      <main class="flex-1">
        <div class="mx-auto max-w-3xl px-4 py-12">
          ${navHtml}
          <h1 class="font-serif text-2xl font-bold text-foreground mb-6">${policy.title}</h1>
          <div class="prose-policy space-y-4 text-sm leading-relaxed text-foreground/90">
            ${policy.contentHtml}
          </div>
        </div>
      </main>
      <footer class="border-t border-sidebar-border bg-sidebar text-sidebar-foreground p-6 text-center text-xs text-muted-foreground">
        © ${new Date().getFullYear()} FarmFreshFarmer. All rights reserved.
      </footer>
    </div>
  `;

  return indexTemplate.replace('<div id="root"></div>', `<div id="root">${bodyHtml}</div>`);
}

export async function prerenderPolicies() {
  ...
  for (const policy of POLICIES) {
    const pageHtml = buildPolicyPageHtml(indexHtml, policy);
    
    // Create directory and index.html (e.g. dist/public/terms/index.html)
    const dirPath = path.join(distPublic, policy.slug);
    await fs.mkdir(dirPath, { recursive: true });
    await fs.writeFile(path.join(dirPath, "index.html"), pageHtml, "utf-8");

    // Also write slug.html (e.g. dist/public/terms.html)
    await fs.writeFile(path.join(distPublic, `${policy.slug}.html`), pageHtml, "utf-8");
  }

  console.log(`Pre-rendered ${POLICIES.length} static legal policy pages successfully into dist/public/`);
}
