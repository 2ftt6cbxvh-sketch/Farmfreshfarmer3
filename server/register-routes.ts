...
async function isPrimaryAdminUser(req: Request): Promise<boolean> {
  ...
  app.delete("/api/admin/users/:id/permanent", requireAdmin, h(async (req, res) => {
    ...
    const result = await purgeUserCompletelyFromDatabase(targetId, adminUser?.id);

    res.json({
      success: true,
      message: `User ${result.name} (ID #${targetId}) and all associated records permanently purged from the database. 🗑️`,
    });
  }));
