...
function PolicyShell({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  const hours = publicSettings?.operating_hours || "Monday - Friday (9:00 - 18:00 IST)";
    ...
    <Layout>
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        <div className="mt-10 rounded-xl border border-emerald-500/30 bg-card p-5 text-sm text-muted-foreground shadow-md">
          <p className="font-bold text-foreground text-base">📞 Contact Us & Platform Support</p>
          <p className="mt-2 text-foreground/90">📍 {business} · {address}</p>
          <p className="mt-1">📱 Phone / Support Contact: <span className="font-semibold text-emerald-400">{phone}</span> (9866399514)</p>
...
/* ------------------------------------------------------------------ */
/* 1. Terms & Conditions (Official PhonePe Business Template)         */
/* ------------------------------------------------------------------ */
export function TermsPage() {
  ...
  const phone = publicSettings?.contact_phone || "9866399514";
  const city = publicSettings?.store_city || "Vijayawada";
  const state = publicSettings?.store_state || "Andhra Pradesh";
  ...
  return (
    <PolicyShell title="Terms & Conditions">
      ...
      <p>
        2. This document is published in accordance with the provisions of Rule 3 (1) of the Information Technology (Intermediaries guidelines) Rules, 2011 that require publishing the rules and regulations, privacy policy and Terms of Use for access or usage of domain name <strong>https://www.farmfreshfarmer.com/#/privacy</strong> ('Website'), including the related mobile site and mobile application (hereinafter referred to as 'Platform').
      </p>
      <p>
        3. The Platform is owned by <strong>{phone}</strong>, a company incorporated under the Companies Act, 1956 with its registered office at <strong>{city}, {state}</strong> (hereinafter referred to as 'Platform Owner', 'we', 'us', 'our').
      </p>
      <p>
      ...
      <ol className="list-decimal pl-6 space-y-3">
        ...
        <li>
          All disputes arising out of or in connection with these Terms shall be subject to the exclusive jurisdiction of the courts in {city}, {state}.
        </li>
        <li>
...
/* ------------------------------------------------------------------ */
/* 2. Privacy Policy (Official PhonePe Business Template)             */
/* ------------------------------------------------------------------ */
export function PrivacyPage() {
  ...
  return (
    <PolicyShell title="Privacy Policy">
      ...
      <div className="rounded-xl border border-emerald-500/30 bg-card p-4 space-y-1 text-sm">
        <p><strong>Name of the Office / Grievance Officer:</strong> Compliance & Grievance Officer</p>
        <p><strong>Designation:</strong> Chief Grievance Redressal Officer ({business})</p>
        <p><strong>Name and Address of Company / Entity:</strong> 9866399514, {city}, {state}, India</p>
        <p><strong>Contact Us:</strong> Phone: {phone} | Email: {email}</p>
        <p><strong>Operating Hours:</strong> Monday - Friday (9:00 - 18:00 IST)</p>
      </div>
    </PolicyShell>
  );
}
...
/* ------------------------------------------------------------------ */
/* 3. Refund and Cancellation policy (Official PhonePe Template)      */
/* ------------------------------------------------------------------ */
  ...
  return (
    <PolicyShell title="Refund and Cancellation policy">
      ...
      <ol className="list-decimal pl-6 space-y-3">
        ...
        <li>
          In case of any refunds approved by <strong>{phone}</strong>, it will take <strong>7 days</strong> for the refund to be processed to you.
        </li>
      </ol>
    </PolicyShell>
  );
...
/* ------------------------------------------------------------------ */
/* 4. Return Policy (Official PhonePe Business Template)              */
/* ------------------------------------------------------------------ */
...
export function GrievancePage() {
  ...
  return (
    <PolicyShell title="Grievance Redressal">
      ...
      <H2>Grievance & Support Officer</H2>
      <div className="rounded-xl border-2 border-emerald-500/40 bg-emerald-50/50 dark:bg-emerald-900/10 p-5 space-y-2 text-sm">
        <p className="font-bold text-base text-emerald-700 dark:text-emerald-400">
          🏛️ Designated Grievance Officer
        </p>
        <p><strong>Entity Name:</strong> 9866399514 ({business})</p>
        <p><strong>Office Address:</strong> Vijayawada, Andhra Pradesh, India</p>
        <p><strong>Contact Email:</strong> <a href={`mailto:${email}`} className="text-emerald-600 underline">{email}</a></p>
        <p><strong>Contact Phone:</strong> {phone}</p>
        <p><strong>Support Operating Hours:</strong> Monday - Friday (9:00 - 18:00 IST)</p>
      </div>
    </PolicyShell>
  );
}
