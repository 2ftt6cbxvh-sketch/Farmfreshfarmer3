...
export default function AdminProducts() {
  ...
  async function handleRegenerateOnlyImage() {
    if (!form.name.trim()) {
      toast({
        title: "Product name required",
    ...
    setIsAiGeneratingStudio(true);
    try {
      const res = await apiRequest("POST", "/api/admin/products/generate-image-now", {
        name: form.name.trim(),
        categorySlug: form.categorySlug || "fruits",
      });
      const data = await res.json();
      if (data?.image) {
        ...
        toast({
          title: "🎨 Live Image Generated by Mac mini M4!",
          description: `Rendered bespoke 1024x1024 food photography on M4 GPU.`,
        });
      }
    } catch (err: any) {
      toast({
        title: "Generation Notice",
        description: "Rendering fallback image or Mac mini is currently processing another batch.",
        variant: "destructive",
      });
    } finally {
      setIsAiGeneratingStudio(false);
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="p-4 rounded-3xl bg-card border-2 border-emerald-500/20 shadow-md space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={handleRegenerateOnlyImage}
                    disabled={isAiGeneratingStudio || !form.name.trim()}
                  ...
                  >
                    {isAiGeneratingStudio ? (
                      <>
                        ...
                        <span>Rendering on Mac mini M4...</span>
                      </>
                    ) : (
                      <span>🔄 Regenerate AI Photo</span>
                    )}
                  </button>
                </div>
