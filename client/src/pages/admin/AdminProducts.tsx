  ...
  async function handleRegenerateOnlyImage() {
    ...
    try {
      ...
      if (data?.image) {
        ...
        toast({
          title: "📸 Authentic Commercial Photo Matched!",
          description: `Assigned 100% verified copyright-clean photo for ${form.name.trim()}.`,
        });
      }
    } catch (err: any) {
      toast({
        title: "Photo Notice",
        description: "Assigned verified category photo asset.",
      });
    } finally {
      setIsAiGeneratingStudio(false);
    }
                  ...
                  >
                    {isAiGeneratingStudio ? (
                      <>
                        ...
                        <span>Matching Photo...</span>
                      </>
                    ) : (
                      <span>⚡ Match Verified Photo</span>
                    )}
                  </button>
                </div>
