    ...
    <ScrollView style={[styles.container, { backgroundColor: bg }]} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
      ...
      <View style={styles.sectionContainer}>
        {(() => {
          ...
          const activeRules = (starRules as any[])
            .filter((r: any) => r.ruleType === targetRuleType && r.active)
            .sort((a: any, b: any) => a.starFrom - b.starFrom);

          const displayRules = activeRules.length > 0
            ? activeRules
            : (isStaffUser
                ? [
          ...
          return (
            <>
              <Text style={[styles.sectionHeading, { color: mutedColor }]}>
                {isStaffUser ? '🛡️ STAFF AUTHORIZATION TIERS (1–6 STARS)' : 'VIP LOYALTY REWARDS (1–5 STARS)'}
              </Text>
      ...
      <View style={styles.versionBadgeRow}>
        <View style={styles.versionBadgeInner}>
          ...
          <Text style={styles.versionBadgeText}>App Build v10.1.0 · Security Core v10.1.0</Text>
        </View>
      </View>
      <Text style={[styles.footerText, { color: mutedColor }]}>{storeName} v10.1.0 · {email}</Text>
